...
1. Check the tool catalog for site-listing and update-inventory capabilities, then scope the audit to the sites in `$ARGUMENTS`, or to all managed sites when it is empty.
2. Check WordPress core versions and call out anything outdated or past its supported line.
...
- Use only the sites named in `$ARGUMENTS`. Never synthesize site IDs, add sites the user did not ask for, or drop sites to make the report tidier.
