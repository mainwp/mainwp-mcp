...
| `CONFIRMATION_UNSUPPORTED` | error  | The ability is destructive (or unannotated, which counts as destructive) and declares no usable `confirm` parameter, so the required flow cannot run.                                                                                                                                  | Stop. Tell the user the Dashboard must annotate the ability correctly, or the operation must be allowed explicitly through tool filtering. No argument combination gets past this.      |
