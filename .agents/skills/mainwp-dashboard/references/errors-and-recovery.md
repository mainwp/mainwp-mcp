...
| `PREVIEW_REQUIRED`         | error  | No usable token backs this confirmed call. Covers a missing `confirmation_token`, an unknown or already-used token, a token issued for a different tool or Dashboard identity, arguments that changed since the preview, and a destructive call with no confirmation arguments at all. | If you hold a valid token and simply left it out, resend the confirmed call with it. Otherwise restart the flow at the preview step with the arguments you actually intend to run: a mismatched token (wrong tool, identity, or arguments) was deleted by the failed check. |
...
| -32029 | `RATE_LIMITED`       | Upstream 429.                                                                                                                 | Abilities annotated `readonly` are retried automatically with backoff. If it still fails, reduce fan-out and space the work out.                                                       |
...
| -32000 | `SERVER_ERROR`       | Dashboard returned 5xx.                                                                                                       | Abilities annotated `readonly` were already retried. Report the failure with the sanitized message; do not paper over it with a different tool.                                        |
